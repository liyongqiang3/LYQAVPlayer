...
void *KVO_Context_TYVideoPlayerController = &KVO_Context_TYVideoPlayerController;

void TYVideo_RemoveKVOObserverSafely(id target, id observer, NSString *keyPath)
{
    if (!target || !observer) {
        return;
    }
...
@implementation TYVideoPlayerManger
...
- (id)init
{
    ...
    if (self) {
        ...
        _scalingMode = TYVideoScaleModeAspectFit;
        _rotateType = TYVideoRotateTypeNone;
        _muted = NO;
        _playbackRate = 1.0f;
        _ignoreAudioInterruption = YES;
        _playbackState = TYVideoPlaybackStateStopped;
...
{
    if (TYVideo_isEmptyArray(times) || !block) {
        return;
    }
 ...
 completionHandler:(void(^)(BOOL finish))completionHandler
{
    if (!self.player) {
        if (error) {
            *error = [NSError errorWithDomain:TYVideoPlayerErrorDomain
                                         code:TYVideoPlayerErrorPlayerNil
...
{
    if (!audioURL || TYVideo_isEmptyArray(times)) {
        return;
    }
...
{
    if (context != KVO_Context_TYVideoPlayerController) {
        ...
        if ([path isEqualToString:NSStringFromSelector(@selector(status))]) {
            ...
            switch (status)
            {
                ...
                case AVPlayerItemStatusFailed: {
                    NSError *error = self.currentItem.error ? : TYError(TYVideoPlayerErrorPlayerItemStatusFailed, nil);
                    
                    //                    TY_VIDEO_ERROR(@"%@ player item status failed: error = %@", self.currentItemKey, error);
            ...
            if (isPlaybackLikelyToKeepUp) {
                //                TY_VIDEO_DEBUG(@"%@ isPlaybackLikelyToKeepUp", self.currentItemKey);
            }
            ...
            if (isPlaybackBufferFull) {
                //                TY_VIDEO_DEBUG(@"%@ isPlaybackBufferFull", self.currentItemKey);
            }
            ...
            //            TY_VIDEO_INFO(@"%@ loadedTimeRanges: %@", self.currentItemKey, stringForLoadedTimeRanges(loadedTimeRanges));
...
- (void)_addPlaybackPollingTimer
{
    ...
    __weak typeof(self) weakSelf = self;
//    self.pollingTimer = [NSTimer TY_video_scheduledTimerWithTimeInterval:0.1 repeats:YES block:^(NSTimer *timer) {
//        __strong typeof(weakSelf) strongSelf = weakSelf;
...
//        }
//    }];
    self.pollingTimer = [NSTimer timerWithTimeInterval:0.1 repeats:YES block:^(NSTimer * _Nonnull timer) {
        __strong typeof(weakSelf) strongSelf = weakSelf;
        if (   strongSelf.currentItem
            && strongSelf.isPreparedToPlay) {
            strongSelf.currentPlaybackRate = CMTimebaseGetRate(strongSelf.currentItem.timebase);
...
- (void)_addAVAudioSessionObservers
{
    ...
    [[NSNotificationCenter defaultCenter] addObserverForName:AVAudioSessionInterruptionNotification object:[AVAudioSession sharedInstance] queue:[NSOperationQueue mainQueue] usingBlock:^(NSNotification * _Nonnull note) {
        ...
        if ([interruptType isEqualToNumber:@(AVAudioSessionInterruptionTypeBegan)]) {
            if (strongSelf.state == TYVideoPlayerStatePlay) {
...
{
    self.contentView = [[TYVideoPlayerView alloc] initWithFrame:[UIScreen mainScreen].bounds];
    self.contentView.playerManger = self;
}
...
- (void)_onReadyToPlay
{
    dispatch_async_on_main_queue(^{
        if (!self.isPreparedToPlay) {
            ...
            //            TY_VIDEO_INFO(@"%@ isPreparedToPlay", self.currentItemKey);
...
- (void)setContentURL:(NSURL *)contentURL
{
    ...
    self.cachePlayURL = [TYURLTransformer customURLForOriginURL:contentURL];
    self.currentItemKey = TYVideoURLStringToCacheKey(contentURL.absoluteString);
    
    //    TY_VIDEO_INFO(@"%@ setContentURL: URL = %@", self.currentItemKey, contentURL.absoluteString);
}
...
- (void)setPlaybackRate:(float)playbackRate
{
    ...
    if (self.player && self.state == TYVideoPlayerStatePlay) {
...
- (void)setPlaybackState:(TYVideoPlaybackState)playbackState
{
    if (_playbackState != playbackState) {
        if (playbackState == TYVideoPlaybackStatePlaying) {
            [self _invalidatePlaybackPollingTimer];
        } else {
            [self _addPlaybackPollingTimer];
        }
        
        TYVideoPlaybackState originPlaybackState = _playbackState;
        
        //        TY_VIDEO_INFO(@"%@ playbackState change: %@ -> %@",
        //                       self.currentItemKey,
...
- (void)setCurrentPlaybackRate:(double)currentPlaybackRate
{
    if (!FLOAT_EQUAL_ZERO(currentPlaybackRate - _currentPlaybackRate)) {
        if (   FLOAT_EQUAL_ZERO(_currentPlaybackRate)
            ...
            self.playbackState = TYVideoPlaybackStatePlaying;
        }
